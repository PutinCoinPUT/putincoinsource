...
{
    return GetBoolArg("-testnet", false) ? 18568 : 8568;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop PutinCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "PutinCoin server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: putincoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("putincoin-rpclist");
...
void ThreadRPCServer2(void* parg)
{
    ...
    if ((mapArgs["-rpcpassword"] == "") || (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use putincoind";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("putincoin-rpchand");
